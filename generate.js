 ...
 *
 * Emits a Lottie (bodymovin v5) JSON: a data-lineage graph, drawn complete and
 * still, with data flowing along its edges on a 5s seamless loop. The graph
 * itself never animates — only the dots move, and the nodes they land on.
 *
...
const CONFIG = {
  ...
  height: 450,
  fps: 60,
  loopEnd: 300,    // the whole animation is one seamless loop, [0, loopEnd)
  // Position samples per dot per crossing. Fewer keyframes means a smaller
  // file and less interpolation work per frame; below ~10 the polyline
  // approximation of the curve starts to show.
  samples: 12,
  colors: {
...
const { width: W, height: H, loopEnd } = CONFIG;
const LOOP_LEN = loopEnd;
 ...
 */
const NODES = [
  { id: 'src1', kind: 'source', glyph: 'rows', x: 100, y: 92 },
  { id: 'src2', kind: 'source', glyph: 'rows', x: 106, y: 214 },
  { id: 'src3', kind: 'source', glyph: 'rows', x: 112, y: 352 },
  { id: 'tf1', kind: 'transform', glyph: 'dot', x: 314, y: 142 },
  { id: 'tf2', kind: 'transform', glyph: 'dot', x: 314, y: 288 },
  { id: 'ds', kind: 'dataset', glyph: 'core-rows', x: 520, y: 218 },
  { id: 'dash', kind: 'consumer', glyph: 'bars', x: 696, y: 122 },
  { id: 'rep', kind: 'consumer', glyph: 'lines', x: 696, y: 318 },
];
 ...
 */
const EDGES = [
  { from: 'src1', to: 'tf1', flow: { cycle: 150, travel: 96, size: 3.2 } },
  { from: 'src2', to: 'tf1', flow: { cycle: 100, travel: 74, size: 3.6 } },
  { from: 'src2', to: 'tf2', flow: { cycle: 150, travel: 92, size: 3.2 } },
  { from: 'src3', to: 'tf2', flow: { cycle: 300, travel: 150, size: 2.8 } },
  { from: 'tf1', to: 'ds', flow: { cycle: 100, travel: 72, size: 3.8 } },
  { from: 'tf2', to: 'ds', flow: { cycle: 150, travel: 88, size: 3.4 } },
  { from: 'ds', to: 'dash', flow: { cycle: 75, travel: 58, size: 4.2 } },
  { from: 'ds', to: 'rep', flow: { cycle: 150, travel: 86, size: 3.4 } },
];
...
/**
 * Scale keyframes for a node: one small nudge every time a dot lands on it.
 * Each edge's traffic is periodic over the loop, so the rhythm is too — but
 * the periods differ (75/100/150/300), so the nudges never settle into a
 * metronome. A node with no reaction configured gets no keyframes at all.
 *
...
function nodeScaleSteps(n) {
  ...
  const phases = [...new Set(
    EDGES
      ...
      .map((a) => (a % LOOP_LEN + LOOP_LEN) % LOOP_LEN)
  )]
...
function nodeLayer(n) {
  const scaleSteps = nodeScaleSteps(n);
  return layer({
    nm: `node-${n.id}`,
    ip: 0,
    ks: {
      ...
      p: stat([n.x, n.y, 0]),
      a: stat([0, 0, 0]),
      s: scaleSteps ? anim(scaleSteps, EASE.out) : stat([100, 100, 100]),
    },
    ...
    shapes: [
      ...(drawsArrow ? [{
        ...
        it: [
          ...
          fill(C.arrow),
          groupTransform({ p: stat(g.p3), r: stat(angle) }),
        ],
      }] : []),
      {
        ty: 'gr', nm: `edge${idx}`,
...
function flowStarts(e, idx) {
  ...
  const starts = [];
  for (let t0 = phase - cycle; t0 <= loopEnd + cycle; t0 += cycle) starts.push(t0);
  return starts;
}
 ...
 * frame 0 is identical to the state at `loopEnd` — which is what makes the
 * loop seamless. Per-edge cycles make the graph read as pipes with different
 * throughput rather than one metronome.
 *
 * Cycles are emitted starting one period *before* frame 0, so a dot already
 * in flight when the loop starts is drawn mid-flight rather than popping in.
 */
...
const animation = {
  ...
  assets: [],
  layers,
  markers: [{ tm: 0, cm: 'loop', dr: LOOP_LEN }],
};
